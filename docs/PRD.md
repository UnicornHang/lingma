...
**停靠点**：新建向导与开书流水线停在细纲，不自动写正文；用户点「写第 1 章」才进入 Writer。无对应细纲时拦截（HTTP 409 / WS FAILED）。写完一章后自动提交账本增量。痕迹检测与模型润色分开：检测可标阻断，润色不承诺过检测器。
...
#### 4.11.6 拆文 Agent（后置，先记录不做）

用户导入百万字小说 → 分章抽取并汇总**卷纲 / 主线 / 支线** → 人工调整骨架 → 再生成一本**结构大体相似、角色名与情节已变形**的新书大纲。

- 职责边界：只做故事骨架迁移与再创作大纲；不写新书正文
- 与账本：拆文结果标「对标来源 / 待确认」，不得写入本作品连续性账本当已发生事实
- 现状：**文档登记，当前迭代不实现**

#### 4.11.7 仿文 Agent（MVP 已落地：学文风再写）

用户导入小说 → 学习**文笔、文风** → 生成风格记忆 → 写自己的书时按该风格生成。

与拆文区别：仿文不管主支线剧情树，只管「怎么写得像」。

推荐实现（大文本必须分块）：

1. **导入分章**：不一次读全书进模型  
2. **抽风格画像**（短 JSON）：句长、对话密度、修辞习惯、节奏标签、人称/距离感等  
3. **风格片段库**：脱敏/降剧情的短样本入向量库，按场景标签召回  
4. **写前装配**：约束锁 + 细纲 + 账本 + 风格画像 + 少量风格片段 → Writer；提示中明确「学技法，不抄专有名词与桥段」  
5. **与 Editor**：Editor 改已有稿；仿文是写前注入目标文风  

产品约束：用户自备文本；UI 标明参考学习、禁止整章原文长期塞进系统提示。

**MVP 现状（已实现）**：作品详情页粘贴样本（200–20000 字，分析窗约 12000）→ `StyleMimicAgent` 生成画像并挂到 `style_profiles` → 启用后 Writer 注入【仿文风格记忆】slot。片段暂存 JSON（非向量库）；全书批量学习与向量召回后置。
#### 4.8.4 流式输出
