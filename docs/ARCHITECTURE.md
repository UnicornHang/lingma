...
| **拆文**（后置，先记录不做） | 分析 | 导入百万字小说 → 抽卷纲/主线/支线骨架 → 人工调整后变形生成**新书大纲** | 分章文本 | 结构化骨架（标「对标来源/待确认」） |
| **仿文**（MVP） | 分析+记忆 | 粘贴样本 → 学习文笔/文风 → 写入风格画像 → Writer 写前注入 | 分章/粘贴文本 | 风格画像 + 短技法片段（非剧情真相） |

> **拆文 vs 仿文**：拆文迁的是**故事骨架**（主支线拓扑）；仿文迁的是**写法**（句式、节奏、对话密度）。二者都不得把对标书剧情写入本作品连续性账本。

### 4.1.1 仿文 Agent 怎么做（设计口径）

目标：用户导入一本（可很长的）小说，系统学会其**文笔与文风**，生成可复用的「风格记忆」，之后写自己的书时按该风格生成正文。

**不能**把百万字一次塞进 LLM，也**不能**用整章原文当长期系统提示。推荐流水线：

```
导入分章
  → 抽样/滑窗抽特征（句长、对话占比、修辞、节奏标签、短声线样本）
  → 汇总成「风格画像」JSON（全书级，短）
  → 切块向量入库（风格片段，带标签；剧情实体脱敏或降权）
  → 写前：细纲+账本 + 风格画像短卡 + 少量检索到的风格片段
  → Writer 明确：学技法与节奏，不抄专有名词与桥段
```

| 记忆层 | 存什么 | 谁读 |
|--------|--------|------|
| 风格画像（结构化，短） | 叙事人称偏好、平均句长、对话密度、情绪推进习惯、禁用/常用修辞标签 | Writer 写前必带 |
| 风格片段库（向量） | 去剧情化的短段落/句群，按「战斗/对话/写景」等标签 | RAG 按本章场景召回 ≤N 条 |
| 连续性账本 | 仍只记**本作品**已发生事实 | 不被仿文覆盖 |

与 Editor 分工：Editor 是改**已写成的稿**；仿文是写前注入**目标文风**。与拆文分工：拆文产出大纲骨架（后置）；仿文不产出主支线剧情树。

落地顺序建议：① 粘贴/导入数章 → 生成风格画像；② 画像挂到作品或「风格预设」；③ Writer slot 增加「风格记忆」；④ 再扩展全书分章批量学习。

**MVP 已完成 ①–③**：`StyleProfile` 表 + `POST /works/{id}/style-mimic/analyze` + Writer【仿文风格记忆】；④ 与向量片段库后置。
### 4.2 编排流水线
...
| V3 | 自适应编排（按任务选 Agent；开书默认停细纲） | ✅ P1：停细纲 / 运行时状态可编辑 / 知情范围进 Critic / 检测与润色拆开 |
...
v1.0 (3-4 月)
 ...
 ├─ ✅ 仿文 Agent MVP：风格画像挂作品 + Writer【仿文风格记忆】（向量片段库后置）
 ├─ ⏳ 仿文进阶：全书分章批量学习 + 风格片段向量召回
 ├─ ⏳ LangGraph 编排器
 └─ ⏳ 多用户、多设备（同步协议）

v1.x 后置（先记录，当前不做）
 └─ ⏳ 拆文 Agent：百万字导入 → 主线/支线/卷纲骨架 → 人工调整 → 变形生成新书大纲
...
| **拆文 Agent** | （后置）从对标长篇抽出主支线/卷纲骨架，再变形为新书大纲；不写正文 |
| **仿文 Agent** | 学习对标文文笔文风，生成风格画像与片段记忆，供 Writer 按风格写 |
